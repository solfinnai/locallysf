...
export default function HomePage() {
  ...
  return (
    <main className="bg-[#F2F0ED]">
      ...
      <section className="relative overflow-hidden" style={{ height: '65vh', minHeight: '520px' }}>
        ...
        <nav className="relative z-20 flex items-center justify-between px-8 lg:px-16 py-5 max-w-[1400px] mx-auto w-full">
          <div className="flex items-center gap-3">
            <span className="text-white text-sm font-medium" style={{ fontFamily: 'var(--font-display)' }}>◎ LocallySF</span>
        ...
        <div className="relative z-10 flex flex-col items-center justify-center px-6" style={{ height: 'calc(100% - 160px)' }}>
          <h1 className="text-white text-center leading-[0.85]"
            style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(56px, 10vw, 120px)', fontWeight: 700 }}>
            Discover
          </h1>
      ...
      {/* Search bar — positioned to overlap hero bottom */}
      ...
      {/* ═══════════ SOCIAL PROOF BAR ═══════════ */}
      <section className="bg-[#F2F0ED] py-5 border-b border-[#E5E5E5]">
        <div className="max-w-[1100px] mx-auto px-6 flex items-center gap-6 text-xs text-[#777]">
      ...
      <section className="py-20 md:py-28">
        <div className="max-w-[800px] mx-auto px-6 text-center">
          <h2 className="text-[#1A1A1A] leading-[1.15]"
          ...
          <div className="flex items-center justify-center gap-2 mt-7">
            ...
            <div className="w-28 border-t border-dashed border-[#CCC]" />
      ...
      {/* ═══════════ EDITORIAL — "Not Your Boring Directory" ═══════════ */}
      <section className="pb-20 md:pb-28">
        <div className="max-w-[1100px] mx-auto px-6">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            {/* Left text */}
            <div>
              <span className="inline-block border border-[#1A1A1A] text-[10px] font-semibold uppercase tracking-widest px-3 py-1 rounded-full mb-6">
                ✦ 01 &nbsp; Our Mission
              </span>
              <h3 className="text-[#1A1A1A] mb-5 leading-tight"
                style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(24px, 3vw, 36px)', fontWeight: 700 }}>
                Not Your Boring<br />City Directory
              </h3>
              <p className="text-[#777] text-sm leading-relaxed mb-7 max-w-sm">
              ...
              <Link href="/about" className="inline-block bg-[#1A1A1A] text-white text-sm font-medium px-5 py-2.5 rounded-lg hover:bg-[#333] transition-colors">
                Learn More
              </Link>
            </div>
            {/* Right — offset photos */}
            <div className="relative" style={{ height: '420px' }}>
              {editorial[0] && (
                <div className="absolute top-0 left-0 w-[58%] rounded-xl overflow-hidden shadow-lg z-10" style={{ height: '65%' }}>
                  <img src={editorial[0].photos[0].url} alt={editorial[0].name} className="w-full h-full object-cover" />
              ...
              {editorial[1] && (
                <div className="absolute top-[18%] right-0 w-[48%] rounded-xl overflow-hidden shadow-lg z-20" style={{ height: '50%' }}>
                  <img src={editorial[1].photos[0].url} alt={editorial[1].name} className="w-full h-full object-cover" />
                  <div className="absolute bottom-3 left-3 bg-black/60 backdrop-blur-sm text-white text-xs px-3 py-1 rounded-full">
              ...
              {editorial[2] && (
                <div className="absolute bottom-0 left-[12%] w-[42%] rounded-xl overflow-hidden shadow-lg z-30" style={{ height: '42%' }}>
                  <img src={editorial[2].photos[0].url} alt={editorial[2].name} className="w-full h-full object-cover" />
                  <div className="absolute bottom-3 left-3 bg-black/60 backdrop-blur-sm text-white text-xs px-3 py-1 rounded-full">
      ...
      <section className="py-16 md:py-24">
        <div className="max-w-[1100px] mx-auto px-6">
          ...
          <div className="flex flex-col md:flex-row md:items-end md:justify-between mb-8">
            <div>
              <span className="inline-block border border-[#1A1A1A] text-[10px] font-semibold uppercase tracking-widest px-3 py-1 rounded-full mb-4">
                ✦ Popular Places 2026
              </span>
              <h3 className="text-[#1A1A1A]"
                style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(28px, 4vw, 46px)', fontWeight: 700 }}>
                Pick the Place
              </h3>
            </div>
            <p className="text-[#777] text-xs max-w-[220px] leading-relaxed mt-4 md:mt-0">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
            {featured.map(place => (
              <Link key={place.id} href={`/place/${place.slug}`} className="group bg-white rounded-xl border border-[#E5E5E5] overflow-hidden hover:shadow-xl transition-all duration-300">
                {/* Header */}
                <div className="p-4 pb-2">
                  <div className="flex justify-between items-start">
                    <div>
                      <h4 className="font-bold text-[#1A1A1A] text-base group-hover:text-[#E8A838] transition-colors">{place.name}</h4>
                      <p className="text-[#999] text-xs mt-0.5">{place.subcategory} · {place.neighborhood}</p>
                    </div>
                ...
                {/* Bottom info */}
                <div className="p-4 pt-3 flex items-center gap-4 text-[10px] text-[#999]">
            ...
            {categoryEntries.map(([key, cat]) => {
              ...
              return (
                <Link key={key} href={`/${key}`} className="group relative rounded-xl overflow-hidden aspect-[4/3]">
                  <img src={photo} alt={cat.name} className="absolute inset-0 w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
        ...
        <div className="max-w-[900px] mx-auto px-6">
          ...
          <div className="space-y-2.5">
            {places
              ...
              .slice(0, 8)
              .map((place, i) => (
                <Link key={place.id} href={`/place/${place.slug}`} className="flex items-center gap-4 bg-white rounded-xl p-3.5 border border-[#E5E5E5] hover:shadow-lg hover:-translate-y-0.5 transition-all duration-300 group">
                  ...
                  <div className="flex-1 min-w-0">
                    <h4 className="font-semibold text-[#1A1A1A] text-sm group-hover:text-[#E8A838] transition-colors truncate">{place.name}</h4>
                    <p className="text-[#999] text-xs truncate">{place.subcategory} · {place.neighborhood}</p>
                  </div>
                  <div className="flex items-center gap-1 flex-shrink-0">
                    <span className="text-[#E8A838] text-xs">★</span>
                    <span className="font-bold text-[#1A1A1A] text-sm">{place.rating}</span>
                    {(place.reviewCount > 0 || place.reviews?.length > 0) && (
                      <span className="text-[#999] text-[10px] ml-0.5">({(place.reviewCount || place.reviews?.length || 0).toLocaleString()})</span>
                    )}
                  </div>
                  {place.priceLevel && (
                    <span className="text-[#CCC] text-xs flex-shrink-0 hidden md:block">{'$'.repeat(place.priceLevel)}</span>
                  )}
                  <span className="text-[#CCC] group-hover:text-[#E8A838] transition-colors flex-shrink-0">→</span>
                </Link>
              ))}
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
            {places
              ...
              .map(place => {
                ...
                return (
                  <a key={place.id} href={place.googleMapsUrl || `/place/${place.slug}`} target={place.googleMapsUrl ? '_blank' : '_self'} rel="noopener noreferrer" className="bg-white rounded-xl p-5 border border-[#E5E5E5] hover:shadow-lg transition-all duration-300 block">
                    <div className="flex items-center justify-between mb-3">
                      <div className="text-[#E8A838] text-xs">{'★'.repeat(r.rating)}{'☆'.repeat(5 - r.rating)}</div>
                      <span className="text-gray-300 text-[10px]">via Google ↗</span>
                    </div>
                    <p className="text-[#1A1A1A] text-sm leading-relaxed mb-4 line-clamp-4">
                      &ldquo;{r.text.slice(0, 180)}{r.text.length > 180 ? '…' : ''}&rdquo;
                    </p>
                    <div className="flex items-center gap-2.5">
                      <div className="w-7 h-7 rounded-full bg-[#1A1A1A] flex items-center justify-center text-white text-[10px] font-bold">{r.author[0]}</div>
                      <div>
                        <p className="font-semibold text-xs text-[#1A1A1A]">{r.author}</p>
                        <p className="text-[#999] text-[10px]">on {place.name}</p>
                      </div>
                    </div>
                  </a>
                );
      ...
      <section className="py-16 md:py-24 bg-[#1A1A1A]">
        <div className="max-w-md mx-auto px-6 text-center">
          <h3 className="text-white text-2xl md:text-3xl mb-2" style={{ fontFamily: 'var(--font-display)', fontWeight: 700 }}>
            Stay Local
          </h3>
          <p className="text-white/40 text-xs mb-6">Weekly guides to the best of San Francisco. No spam.</p>
          <div className="flex gap-2">
            <input type="email" placeholder="Your email" className="flex-1 bg-white px-4 py-3 rounded-lg text-sm text-[#1A1A1A] placeholder-[#999] outline-none" />
            <button className="bg-[#E8A838] text-white px-5 py-3 rounded-lg font-semibold text-sm hover:bg-[#d49a30] transition-colors">Subscribe</button>
          </div>
        </div>
      </section>
      ...
      <footer className="bg-[#1A1A1A] border-t border-white/5 py-8">
        <div className="max-w-[1100px] mx-auto px-6 flex flex-col md:flex-row justify-between items-center text-white/20 text-xs">
          <span className="text-white/50 font-medium" style={{ fontFamily: 'var(--font-display)' }}>◎ LocallySF</span>
